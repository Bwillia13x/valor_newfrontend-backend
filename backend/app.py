...
from .rate_limiter import financial_data_rate_limit, rate_limit
...
# Error reporting routes available if needed
    ...
    responses:
      200:
        ...
        schema:
          type: object
      503:
        description: One or more readiness checks failing
    """
    checks = {
        "db": {"ok": False, "detail": ""},
...
@tenant_required
def get_scenarios():
    """Get paginated scenarios for the user with security validation"""
    try:
        ...
        # Start with base query
        query = Scenario.query.filter_by(user_id=user.id)
        
        # Apply pagination with security checks
        paginated_query, pagination_info = apply_pagination(query, Scenario)
        
        # Convert to dict format
        scenarios = [scenario.to_dict() for scenario in paginated_query.items]
        ...
        app.logger.error(f"Failed to fetch scenarios: {str(e)}", extra={
